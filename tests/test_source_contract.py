...
class RandomFlipSwiftSourceContract(unittest.TestCase):
    ...
    def test_roothide_build_contract(self) -> None:
        ...
        self.assertRegex(control, r"(?m)^Version: 0\.0\.9~diag1$")
        self.assertRegex(makefile, r"(?m)^PACKAGE_VERSION = 0\.0\.9~diag1$")
        ...
        self.assertIn('PACKAGE_VERSION: "0.0.9~diag1"', workflow)
        self.assertIn('grep -Fqx "Version: ${PACKAGE_VERSION}" control', workflow)
        self.assertIn('grep -Fqx "PACKAGE_VERSION = ${PACKAGE_VERSION}" Makefile', workflow)
        self.assertIn("- 当前候选版本：RootHide 诊断包 `0.0.9~diag1`（基线 `0.0.8`）", readme)
        self.assertIn("Infinity Drift / Figure Eight（∞ 漂移 / 8 字巡航）", readme)
        self.assertIn("`0.0.4` 至 `0.0.8` 的图标动画均已通过实机验证", readme)
        self.assertIn("PACKAGE_VERSION=0.0.9~diag1", readme)
    ...
    def test_public_source_contains_no_embedded_secrets(self) -> None:
        text = "\n".join(
            ...
            and path.suffix.lower() in {".swift", ".m", ".xm", ".h", ".md", ".plist", ".yml", ".yaml", ""}
        )
