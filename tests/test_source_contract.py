...
class RandomFlipSwiftSourceContract(unittest.TestCase):
    ...
    def test_roothide_build_contract(self) -> None:
        ...
        self.assertRegex(control, r"(?m)^Version: 0\.0\.9~diag3$")
        self.assertRegex(makefile, r"(?m)^PACKAGE_VERSION = 0\.0\.9~diag3$")
        ...
        self.assertIn('PACKAGE_VERSION: "0.0.9~diag3"', workflow)
        ...
        self.assertIn("- 当前候选版本：RootHide 诊断包 `0.0.9~diag3`（基线 `0.0.8`）", readme)
        self.assertIn("Infinity Drift / Figure Eight（∞ 漂移 / 8 字巡航）", readme)
        self.assertIn("`0.0.4` 至 `0.0.8` 的图标动画均已通过实机验证", readme)
        self.assertIn("PACKAGE_VERSION=0.0.9~diag3", readme)
