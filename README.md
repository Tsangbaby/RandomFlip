...
这是由 tsangbaby 维护的 `RandomFlip` Swift 现代化重构版。核心行为是：在桌面空闲时，随机选择一个当前可见的主屏幕或 Dock 图标，执行翻转、弹跳、角度摇摆、轻旋转、水平震动、跳起落地、果冻挤压、轨道螺旋、Flutter / Leaf 风吹叶片或 Infinity Drift / Figure Eight（∞ 漂移 / 8 字巡航）动画。
...
- 当前候选版本：RootHide 诊断包 `0.0.9~diag4`（基线 `0.0.8`）
...
- 实机验证：`0.0.4` 至 `0.0.8` 的图标动画均已通过实机验证；`diag1` 已取得 iOS 15.4.1 证据，`diag2`/`diag3` 已闭合 iOS 16.0.3 写入与能力矩阵，`0.0.9~diag4` 仅用于确认真实 modifier 与几何方法调用
...
```sh
# RootHide
make clean package FINALPACKAGE=1 PACKAGE_VERSION=0.0.9~diag4 THEOS_PACKAGE_SCHEME=roothide
```
...
`0.0.8` 已通过 RootHide 包级验证和实机验证。`0.0.9~diag4` 必须先完成源码、RootHide 包级验证，再分别在 iOS 15.4.1 与 iOS 16.0.3 按 `DIAGNOSTICS.md` 各采样一次；诊断结果不能表述为正式“顶部磁吸收束”功能已经实现或通过实机验证。
