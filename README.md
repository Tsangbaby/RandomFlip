...
- 当前候选版本：RootHide 诊断包 `0.0.9~diag3`（基线 `0.0.8`）
...
- 实机验证：`0.0.4` 至 `0.0.8` 的图标动画均已通过实机验证；`0.0.9~diag1` 已取得 iOS 15.4.1 证据，`0.0.9~diag2` 已把 iOS 16.0.3 无主日志定位为共享 Preferences 目录验证失败，`0.0.9~diag3` 改用专属私有目录继续采集
...
```sh
# RootHide
make clean package FINALPACKAGE=1 PACKAGE_VERSION=0.0.9~diag3 THEOS_PACKAGE_SCHEME=roothide
```
...
`0.0.8` 已通过 RootHide 包级验证和实机验证。`0.0.9~diag3` 必须先完成源码、RootHide 包级验证，再在 iOS 16.0.3 按 `DIAGNOSTICS.md` 采样；诊断结果不能表述为正式“顶部磁吸收束”功能已经实现或通过实机验证。
